...
contract SmartStorageUnitSystem is System {
  ...
  event Trade(address indexed _ssuOwner, 
              uint256 indexed _ssuSmartObjectId, 
              uint256 _inventoryItemIdIn,
              uint64  calculatedInput, 
              uint256 itemObjectIdOut,
              uint64  calculatedOutput);
  ...
  function execute(uint256 smartObjectId, uint64 quantity, uint256 inventoryItemIdIn) public {
    ...
    emit Trade(ssuOwner, smartObjectId, inventoryItemIdIn, calculatedInput, itemObjectIdOut, quantityOutputItem);
  }
